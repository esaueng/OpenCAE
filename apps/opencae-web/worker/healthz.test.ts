...
const env = new Proxy({} as Parameters<typeof worker.fetch>[1], {
  get(target, key) {
...
for (const suffix of ['', '?probe=monitor']) {
  test(`GET /healthz${suffix} is public, uncached JSON even for navigation requests`, async () => {
    ...
    assert.deepEqual(await response.json(), { status: 'ok', service: 'opencae' });
  });
}

test('HEAD /healthz has the GET headers and no body', async () => {
  const get = await worker.fetch(new Request('https://app.example/healthz'), env);
  const head = await worker.fetch(new Request('https://app.example/healthz', { method: 'HEAD' }), env);
...
for (const method of ['POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS']) {
  test(`${method} /healthz is rejected without touching application services`, async () => {
    const response = await worker.fetch(new Request('https://app.example/healthz', { method }), env);
...
for (const configPath of ["../../../wrangler.jsonc", "../../../wrangler.static.jsonc", "../../../wrangler.retired-do-cleanup.jsonc"]) {
