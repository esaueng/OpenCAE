...
import type { PayloadObjectSelection, PrintLayerOrientation, ResultMode, ResultPlaybackFrameController, ResultPlaybackFrameSnapshot, SectionPlaneState, StressComponent, ThemeMode, ViewerLoadMarker, ViewerSupportMarker, ViewMode } from "../workspaceViewTypes";
...
export function CadViewer(props: CadViewerProps) {
  ...
  return (
    <section className={`viewer-shell ${effectiveViewMode === "results" ? "results-view" : ""}`} aria-label="3D CAD viewer">
      <Canvas frameloop="demand" dpr={viewerDpr} camera={{ position: [4.8, 4.8, 4.8], up: ISO_CAMERA_UP.toArray(), fov: 42 }} onCreated={({ gl }) => { gl.localClippingEnabled = true; }} onPointerMissed={props.onViewerMiss}>
...
function ResultLegend({ resultMode, resultFields, unitSystem, meshSummary, surfaceMesh, showDeformed, deformationScale }: { resultMode: ResultMode; resultFields: ResultField[]; unitSystem: UnitSystem; meshSummary?: MeshSummary; surfaceMesh?: SolverSurfaceMesh; showDeformed?: boolean; deformationScale?: number }) {
  ...
  const title = resultMode === "stress" ? "Von Mises Stress" : resultMode === "displacement" ? "Displacement" : resultMode === "velocity" ? "Velocity" : resultMode === "acceleration" ? "Acceleration" : resultMode === "mode_shape" ? "Normalized Mode Shape" : "Safety Factor";
...
function colorForResult(faces: DisplayFace[], viewMode: ViewMode, resultMode: ResultMode) {
  ...
  return (faceId: string) => {
    ...
    if (resultMode === "displacement" || resultMode === "velocity" || resultMode === "acceleration" || resultMode === "mode_shape") return gradient(value, 28, 142, ["#2f80ed", "#4dd0e1", "#d7f75b"]);
...
function resultPalette(resultMode: ResultMode): { body: string[] } {
  if (resultMode === "displacement" || resultMode === "velocity" || resultMode === "acceleration" || resultMode === "mode_shape") {
    return {
