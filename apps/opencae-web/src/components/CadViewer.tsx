...
import { barycentricPoint, resolveResultProbe, selectActiveResultField, stressComponentForField, type ResolvedResultProbe, type ResultProbeAnchor, type ResultProbePin } from "../resultSelection";
...
import { isSolverSurfaceNodeField, recoverSurfaceNodeScalarField, resultFieldValuesAlignedToGeometry, solverSurfaceResultFields, type SolverSurfaceResultFields } from "../solverSurfaceFields";
...
function SupportGlyph({ kind, marker, face, active, labelPosition, scale = 1 }: { kind: SampleModelKind; marker: ViewerSupportMarker; face: DisplayFace; active: boolean; labelPosition?: [number, number, number]; scale?: number }) {
  ...
  if (holeGeometry) {
    ...
    const color = supportGlyphColor(active);
    const brightColor = active ? "#8cc8ff" : "#7fe7db";
    return (
