...
OpenCAE uses OpenCAE Core Cloud for production structural solves, with local preview behavior kept explicit and separate:
...
- **Preview** is allowed only for explicit local development/demo flows. It uses structured display-bounds proxy meshes and must never be presented as production FEA.
...
Dynamic structural studies generate timed frames with Newmark average-acceleration integration only in explicit local preview flows or Core Cloud production flows. Preview dynamic results cannot be presented as validated FEA for complex geometry.
...
| Bracket without actual volume mesh | Bracket sample display model | Aluminum 6061 | Fixed mounting holes and top face load | Preview solving is rejected with an actual-volume-mesh or OpenCAE Core Cloud diagnostic. |
...
- Complex geometry receives preview solver, `structured_block_proxy`, or `computed_preview` provenance and is displayed as valid FEA.
