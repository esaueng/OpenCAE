...
The convergence panel plots probe displacement and raw element peak von Mises stress against actual DOF. A red capped/skipped marker means that rung was not solved. The verified automatic browser route is guarded at 150,000 DOF. The matrix-free WebGPU operator remains available for development benchmarks, but automatic large-model routing is disabled until the full CG iteration loop is GPU-resident and an end-to-end browser benchmark establishes a release ceiling. OpenCAE reports **apparent convergence** only when all three successful rungs have strictly increasing DOF and the medium-to-fine changes are at most 5% displacement and 10% stress. Failed or non-increasing ladders are inconclusive. Running the ladder does not change the working mesh selection or replace the active results.
...
**Export selected-state CSV** and **Export selected-state VTU** save the canonical raw values for the active run variant and the currently selected static state, dynamic frame, or modal mode. The files identify the Z-up coordinate system, coordinate units, field units and components, variant, and frame/mode identity. VTU uses the canonical Tet4/Tet10 volume connectivity for ParaView-compatible unstructured grids; surface-only nodal fields remain surface-supported and use `NaN` at volume-interior nodes rather than invented values. CSV includes stable one-based node and element identifiers plus explicit connectivity. These v1 exports contain one selected state, not an entire transient series. OpenCAE refuses stale topology and requests estimated to exceed the 128 MB browser-memory budget instead of truncating output.
