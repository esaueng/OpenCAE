...
OpenCAE is a local-first CAD/CAE simulation workspace for structural study setup, fast local solves, and browser-based result review. The current app supports static stress and dynamic structural studies, sample projects, local project files, uploaded geometry previews, deterministic local solver paths, browser-local OpenCAE Core static solves, and report export.

The project is organized around service boundaries so the React workspace, Fastify API, CAD import, meshing, local solvers, browser OpenCAE Core adapters, post-processing, storage, and job runners can evolve independently.
...
The production Cloudflare target for `cae.esau.app` serves the Vite web app from Workers Static Assets. The default deploy path builds the web app, deploys the Worker asset binding, and enables SPA fallback routing. Browser simulations run locally through Detailed local or OpenCAE Core; production deploys do not require `FEA_CONTAINER` or `@cloudflare/containers`.
...
Wrangler uses [wrangler.jsonc](wrangler.jsonc) for the production app domain by default. The legacy container config remains in [wrangler.containers.jsonc](wrangler.containers.jsonc) for reference and explicit experiments only:
...
The old CalculiX/Gmsh container service in `services/opencae-fea-container` is kept for reference. It is no longer part of the default browser run path or production deploy.
...
That static path uses [wrangler.static.jsonc](wrangler.static.jsonc), which targets `opencae-static` and intentionally omits the production custom domain route.

For a local-first/static Worker deploy, use:
...
For Cloudflare Builds, use `pnpm deploy:cloudflare` as the deploy command.
...
- `services/*` - CAD, mesh, solver, post-processing, and legacy container service implementations.
...
- **OpenCAE Core solver** - browser-local CPU Tet4 solving through vendored `@opencae/core` and `@opencae/solver-cpu` packages for eligible static stress studies. Unsupported studies automatically fall back to Detailed local.

The legacy CalculiX/Gmsh container adapter remains in the repo for reference. CalculiX and Gmsh are separately licensed third-party components and are not relicensed under Apache-2.0.
...
OpenCAE is still an engineering preview. Local results are deterministic estimates for product workflow development and should not be treated as certified analysis. Browser-local OpenCAE Core provides the current FEA integration point, while native CAD, meshing, and post-processing support continue to evolve behind the existing service boundaries.
